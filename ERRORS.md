...
- **Fix applied:** `rm -rf build/perseus_interfaces install/perseus_interfaces && colcon build --packages-select perseus_interfaces --symlink-install`, then `source install/setup.bash` *before* building any consumer.
