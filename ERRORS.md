...
### NITROS rectify+apriltag crash was reboot/state-dependent, not a pure code race — 2026-07-09

- **Severity:** Medium
- **Category:** Concurrency
- **File(s):** `software/docker/isaac-ros/nitros-source/apriltag_camera_launch.py`
- **Pattern:** A crash that reproduces 100% of the time within a single boot session (`usb_cam` → `RectifyNode` → `AprilTagNode` on live NITROS frames threw `nvcv::Exception: The tensor handle is null` within ~1s of every launch) was diagnosed as a genuine data race using the standard "does it crash under gdb" timing-perturbation test — gdb masked it, which is real evidence of a race, but was treated as sufficient to conclude the bug was purely in the code path. After a reboot, the byte-identical graph, build, and camera setup ran clean for 30+ minutes with no crash at all, undermining the "100% reproducible" premise the original diagnosis rested on.
- **Root cause:** Likely stale GPU/CUDA driver-level state (fragmented device memory, a corrupted `CudaStreamPool`, or similar) accumulated across many earlier `nvcv::Exception`-terminated `component_container_mt` processes within the same boot session — abnormal C++ exception termination mid-CUDA-op is exactly the kind of exit that can leave device/driver state inconsistent for the *next* process, independent of any race in the NITROS type-adapter code itself. "Crashes every time in this session" and "crashes every time, period" were conflated.
- **Fix applied:** None yet to the underlying NITROS/CUDA behavior (still suspected upstream). Reverted the rectify-skipping workaround in `apriltag_camera_launch.py` back to the full `rectify → apriltag` graph, since a fresh boot ran it clean far longer than the crash previously took to appear. Documented explicitly as provisional, with the one-line revert path noted in the launch file's docstring if the crash returns.
- **Prevention rule:** A "crashes every time" pattern observed only within one long-lived debugging session (especially one involving other crashes of the same process, GPU work, or repeated kill/relaunch cycles) is not proven code-level-deterministic until it's also re-tested from a clean boot. Before concluding a race is purely a code bug based on a single session's reproducibility, retest after a full reboot — cheap to do, and rules out session-accumulated driver/GPU state as the actual variable.
...
### Isaac ROS composable-node remap guessed from the wrong topic-name source — 2026-07-09

- **Severity:** Medium
- **Category:** API Misuse
- **File(s):** `software/docker/isaac-ros/nitros-source/dnn_classify_launch.py`
- **Pattern:** When instantiating an NVIDIA Isaac ROS composable node directly (bypassing NVIDIA's own higher-level `*.launch.py` wrapper), a remap was guessed from that wrapper's *launch argument* name (`tensor_output_topic`, default `'encoded_tensor'`) rather than the node's actual hardcoded wire topic. `DnnImageEncoderNode`'s C++ constructor publishes on the literal string `"tensors"` (`create_publisher<NitrosTensorList>("tensors", ...)`); the wrapper's `tensor_output_topic` parameter is a `launch.py`-level convenience name that only matters if you also use that wrapper's own remap plumbing, not the node's own default topic. The mismatch produced **zero errors** — the encoder ran, received images, and built valid `NitrosTensorList` objects (confirmed via `--log-level debug`: `[Dnn Image Encoder] Image received` plus the full `NitrosBuffer` write/read-handle CUDA-event sequence), it just published to `/tensors` while the downstream node was subscribed to `/tensor_pub`, so nothing downstream ever fired — indistinguishable from a hang or a sync-policy failure without checking topic names directly. A second issue in the same debugging session, `enable_padding: false` immediately crashing the node (`nvcv::Exception: INVALID_DATA_SHAPE`) because the underlying CV-CUDA resize path requires padding when input/output aspect ratios differ, was the more visible failure and got fixed first, momentarily masking that the *silent* topic mismatch was the real remaining blocker.
- **Root cause:** Isaac ROS's public launch-argument names (meant for their own `*.launch.py` composition helpers) don't necessarily match the C++ node's actual default topic strings; assuming they do is an easy, silent mistake when writing a minimal custom launch file instead of using NVIDIA's wrapper.
- **Fix applied:** Bumped the container to `--ros-args --log-level debug` and grepped for `Initializing publisher for topic name` in the log to read the *actual* topic string a newly-wired Isaac ROS node uses, rather than inferring it from a launch-wrapper parameter name. Remapped `('tensors', 'tensor_pub')` instead of `('encoded_tensor', 'tensor_pub')`.
- **Prevention rule:** When composing an NVIDIA Isaac ROS node directly (not through their own launch wrapper), never infer a topic name from a `DeclareLaunchArgument`/parameter name in their sample `*.launch.py` — grep the node's own `.cpp` source for the literal string passed to `create_publisher`/`create_subscription`, or run once at `--log-level debug` and check the `Initializing publisher/subscription for topic name` lines before assuming a remap is correct. A composable-node chain that loads cleanly with no errors and just never produces output is a strong signal to check topic names next, before suspecting QoS or sync-policy issues.

### NVIDIA's aarch64_jetpack70 cuVSLAM binary requires SVE2, which Orin's CPU doesn't have — 2026-07-09

- **Severity:** High
- **Category:** Configuration
- **File(s):** `software/docker/isaac-ros/nitros-source/isaac_ros_nitros/isaac_ros_nitros/lib/cuvslam/lib_aarch64_jetpack70/libcuvslam.so` (gitignored, NVIDIA-published binary — not a file we wrote or can fix)
- **Pattern:** A published binary whose directory name (`aarch64_jetpack70`) exactly matches the target platform (Orin, JetPack 7) is not proof it's actually ISA-compatible with that platform's CPU — the compiler flags used to build it can silently target a newer/different microarchitecture than the one implied by the naming convention. `SIGILL` on the very first `dlopen()` (before any application code runs, inside the library's own ELF constructor) is the signature of this class of bug: a CPU-instruction-set mismatch, not a linking/dependency/driver problem.
- **Root cause:** `libcuvslam.so`'s `lib_aarch64_jetpack70` build contains SVE2 (Scalable Vector Extension 2) instructions (`whilewr`, `whilerw`, `ptrue`, `ld1w`/`st1w` — confirmed via `objdump -d`), executed unconditionally in a static initializer. The Orin's Cortex-A78AE cores (`/proc/cpuinfo` `Features:` line: `asimd` present, `sve`/`sve2` absent) don't implement SVE at all — this is a hardware ISA limitation, not a missing library/driver/kernel config. Confirmed as a `jetpack70`-specific regression, not a general cuVSLAM limitation: the sibling `lib_aarch64_jetpack61` binary (NVIDIA's official JetPack 6.1/Orin target) disassembles with zero SVE/SVE2 instructions. The `jetpack61` binary is itself unusable here for an unrelated reason (`ldd` reports `libcusolver.so.11`/`libcublas.so.12` not found — it wants CUDA 11/12-era sonames incompatible with this host's CUDA 13.2-only install), so there's no published aarch64 cuVSLAM variant that's simultaneously CPU- and CUDA-ABI-compatible with Orin+JetPack7+CUDA13.
- **Fix applied:** None available from our side — cuVSLAM's source isn't published, only this compiled `.so`. Not something a build flag or environment variable on our end can work around, since the illegal instruction is baked into the shared library's own init code. Documented as blocked; recommended reporting to NVIDIA as a build-config bug in their `aarch64_jetpack70` cuVSLAM release.
- **Prevention rule:** When a vendor ships a prebuilt binary named after your exact target platform, don't assume the name guarantees ISA compatibility — verify with a minimal, ROS-free repro (`python3 -c "import ctypes; ctypes.CDLL('path/to/lib.so')"`) before investing time wiring it into a larger system. If it crashes with `SIGILL` specifically (not `SIGSEGV`/missing-symbol errors), check `/proc/cpuinfo`'s `Features:` line against the instructions at the crash PC (`gdb -batch -ex run -ex bt -ex 'x/4i $pc' --args ...`) before assuming it's a build/environment problem on your end — it may be a genuine binary/hardware mismatch nothing on your side can fix.
