...
# push built packages
nix build .#packages.aarch64-linux.default --json | jq -r '.[].outputs | to_entries[].value' | cachix push perseus-lite
nix build .#packages.aarch64-linux.simulation --json | jq -r '.[].outputs | to_entries[].value' | cachix push perseus-lite
