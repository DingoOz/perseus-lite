#!/usr/bin/env bash
# `pixi run -e isaac-nitros test-h264-decode` — Stage 9 proof-of-life
# (decode-only; see isaac-ros-nitros-source-build.md's "Stage 9" section
# for why encode is not available on this Orin Nano SKU). Launches
# h264_decode_launch.py (compressed.h264 -> DecoderNode, hardware NVDEC),
# then runs h264_decode_check.py against it and reports pass/fail.
set -eo pipefail
...
ros2 launch h264_decode_launch.py &
LAUNCH_PID=$!
...
trap 'kill -9 "$LAUNCH_PID" 2>/dev/null || true
      pkill -9 -f "__node:=h264_decode_container" 2>/dev/null || true' EXIT

sleep 5

python3 h264_decode_check.py
