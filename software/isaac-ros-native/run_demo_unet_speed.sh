#!/usr/bin/env bash
# `pixi run -e isaac-nitros demo-unet-speed` — Demo 4 (plan.md): U-Net
# segmentation TensorRT (GPU) vs ONNX Runtime (CPU), direct Python API on
# both sides, no ROS in the timing loop. Prints results, saves
# demo4_result.{json,png}, then publishes the result chart to
# /demo4_unet_comparison for 2 minutes -- view from a laptop on the same
...
python3 -u demo4_unet_speed_compare.py
